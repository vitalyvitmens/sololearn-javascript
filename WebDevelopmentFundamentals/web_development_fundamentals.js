...
//TODO: Layout (Макет)
// Как мы теперь знаем, есть блочные и строчные элементы.
// Блочные элементы охватывают всю ширину своего родителя и начинаются с новой строки, в то время как встроенные элементы размещаются внутри блочных элементов и действуют аналогично их содержимому, располагаясь на той же строке. Например, давайте создадим абзацы текста и выделим в них некоторые слова жирным шрифтом:
// <p> — block element (блочный элемент), а <b> — is inline (встроенный).
{
...
//TODO: Layout (Макет)
// Давайте добавим цвета фона и границы к элементам <p> и <b> , чтобы увидеть, как они расположены на странице:
{
  /* <p>Learning <b>CSS</b> is fun!</p>
<p>some text </p>
<p>another <b>text</b></p> */
...
//TODO: Layout (Макет)
// Свойство display можно использовать для изменения поведения элементов, заставляя их вести себя как блочные или встроенные элементы. Например, давайте заставим встроенные элементы <b> вести себя как блочные элементы:
// Задача: сделать так, чтобы абзацы действовали как встроенные элементы, и проверить результат!
// Напомним, в предыдущем примере мы использовали свойство display, чтобы изменить поведение элементов списка и сделать их встроенными.
// Браво!
{
  /* <p>Learning <b>CSS</b> is fun!</p>
<p>some text </p>
...
//TODO: Layout (Макет)
// Другим значением display является flex.
// Он используется для размещения элементов в строках и столбцах. Рассмотрим три элемента статьи на странице, заключенные в раздел:
// Они являются блочными элементами и отображаются на отдельных строках, каждая из которых занимает всю ширину раздела.
// Мы также добавили несколько цветов и стилей в CSS, чтобы мы могли различать элементы статьи.
{
  /* <section>
...
//TODO: Layout (Макет)
// Теперь, если мы добавим display:flex; к разделу, он заставит статьи внутри него действовать как гибкие элементы и располагаться в столбцах:
// Гибкие элементы имеют равную ширину, чтобы заполнить ширину своего контейнера.
// Задача: Уберите одну из статей и посмотрите, как изменится раскладка.
{
  /* <section>
    <article>First section</article>
    <article>Second section</article>
...
//TODO: Layout (Макет)
// По умолчанию flex-элементы занимают одинаковое место.
// Мы можем контролировать, какую долю пространства занимают гибкие элементы по сравнению с другими элементами, используя свойство flex.
// Например, давайте присвоим первой статье значение гибкости 2, а две другие будут иметь значение гибкости 1:
// Теперь первая статья займет в 2 раза больше места, чем остальные.
// Ты в огне!
{
  /* <section>
...
//TODO: Layout (Макет)
// Давайте создадим реальный макет сайта. Он будет включать верхний и нижний колонтитулы, занимающие всю ширину страницы, и два столбца для основного контента, один из которых занимает в 3 раза больше места, чем другой.
// Макет также будет реагировать и поддерживать структуру для любой ширины браузера.
// Задача: Откройте код и проверьте стили CSS, чтобы понять, как это работает.
{
...
//TODO: Practice: Layout (Практика: Макет)
// Нам нужно изменить макет нашей формы, чтобы поля располагались горизонтально.
// Чтобы получить такой макет, нам нужно сделать следующее:
// 1. Изменить HTML формы: удалить элементы <br>, которые разделяют форму на строки.
// - Добавьте элементы <br> после тегов <label>, чтобы числовое поле отображалось после метки в новой строке.
// - Оберните каждый элемент раздела в теги <div> и предоставьте им class="form-section".
// - Теперь у нас будет 3 элемента "form-section". Два из них будут содержать метки и соответствующие им поля, а второй — кнопку.
// 2. In CSS, set the display property of the form to flex (отображать свойство формы flex).
// 3. Установите для свойства flex значение 1 для класса раздела формы .
// 4. Для нашей кнопки добавьте высоту 40 пикселей и установите ширину на 100% . Подсказка: вот как должен выглядеть flex layout CSS (гибкий CSS-макет):
// form {
//   display: flex;
// }
// .form-section {
...
//TODO: Float (Плавать)
// Свойство float позволяет размещать элемент внутри контейнера.
// Например, мы можем разместить изображение в тексте, чтобы текст обтекал его с одной стороны:
// Поддерживаемые значения — left и right .
// Вы можете плавать любой другой элемент с той же техникой.
{
...
//TODO: Positioning (Позиционирование)
// Для точной настройки положения элементов на странице можно использовать свойство position. Существует ряд различных значений, которые поддерживает свойство position, изменяя способ позиционирования элемента.
// По умолчанию все элементы имеют position:static, что является нормальным потоком.

//TODO: Relative Position (Относительное положение)
// С position: relative; элемент позиционируется относительно своего нормального положения. Свойства top, right, bottom и left могут использоваться для указания того, как будет смещаться элемент. Например:
{
  /* <section>
    <article>First section</article>
...
// }
// Это сдвинет элемент на 30 пикселей снизу.
// Содержимое относительно расположенных элементов можно перемещать и перекрывать другие элементы, но зарезервированное пространство для элемента все еще сохраняется в обычном потоке.

//TODO: Absolute Position (Абсолютное положение)
// Абсолютно позиционированный элемент удаляется из обычного потока страницы.
// Это полезно, например, для создания всплывающих окон, меню управления, панелей и т.д. Например:
// Это сделает позицию элемента на 30 пикселей сверху страницы и на 20 пикселей слева. Позиция элемента определяется относительно первого позиционированного родительского элемента: например, вы можете абсолютно позиционировать элемент внутри относительно позиционированного контейнера.
// Задача: изменить код, чтобы расположить элемент в правом нижнем углу.
{
  /* <section>
    <article>First section</article>
    <article class="special">Second section</article>
...
//TODO: Fixed Position (Фиксированная позиция)
// Фиксированное позиционирование заставляет элемент прилипать к части видимой области, что означает, что он всегда остается на одном и том же месте, даже если страница прокручивается. Например:
// Это приводит к тому, что элемент прилипает к нижней части страницы со смещением на 30 пикселей.
// Задача: изменить код, чтобы элемент занимал всю ширину страницы и оставался сверху.
{
  /* <section>
    <article>First section</article>
    <article class="special">Second section</article>
...
//TODO: z-index (z-индекс)
// Когда элементы расположены вне нормального потока, они могут перекрывать другие элементы.
// Свойство z-index указывает порядок элементов в стеке (какой элемент должен быть размещен впереди или позади других). Например:
// По умолчанию оранжевое поле будет над элементами статьи, однако мы сделали его ниже них, используя более низкое значение z-index.
// Задача: Укажите имя класса для третьей статьи и поместите ее под оранжевую рамку.
{
  /* <section>
    <article>First section</article>
    <article class="special">Second section</article>
...
//TODO: Practice: Positioning (Практика: Позиционирование)
// Изображение в нашем описании выглядит не очень привлекательно.
// Давайте изменим это, а также заставим нашу форму прилипать к нижней части страницы, независимо от ее высоты!
// Задачи:
// 1. Сделать так, чтобы изображение в элементе section плавало влево, а текст обтекал его.
// 2. Дайте изображению отступ в 10 пикселей вправо, чтобы между ним и текстом был зазор.
// 3. Для нашего раздела покупки: сделайте его положение фиксированным в правом нижнем углу страницы. Задайте ему ширину 100%, чтобы он охватывал всю ширину страницы, и установите для свойства box-sizing значение border-box, чтобы размер не выходил за пределы страницы.
// Подсказка: вот как установить фиксированное положение:
// #buy {
//   position: fixed;
//   bottom: 0px;
//   right: 0px;
...
// Это выведет текст "Hello!" консоль.
...
// Задача: изменить код для вывода вашего имени на консоль.
// console.log('Hello!')
...
// Обратите внимание, что каждый оператор обычно заканчивается точкой с запятой, но это не обязательно:
// Задача: изменить код для вывода вашего имени и возраста с помощью отдельных операторов.
// console.log('Welcome')
// console.log('JavaScript is awesome')

//TODO: Comments (Комментарии)
// Помните комментарии в HTML?
// JavaScript также позволяет вам писать комментарии для объяснения кода, добавлять примечания, а также комментировать операторы. Однострочный комментарий начинается с //:
// Все, что находится после // в этой строке, станет комментарием.
// output Hello
// console.log('Hello')

//TODO: Comments (Комментарии)
// Вы также можете создавать многострочные комментарии.
// Они начинаются с /* и заканчиваются */, делая все, что находится между ними, комментарием. Например:
/* this is
 a comment 
regarding my code */
// console.log(42)

//TODO: Console (Консоль)
// Консоль также используется для отображения ошибок в вашем коде.
// Например, давайте сделаем несколько опечаток в коде:
// Запустите код, чтобы увидеть сообщение об ошибке.
// Задача: исправить код, чтобы он приводил к ожидаемому результату.
// // consol.write('Hi')
// console.log('Hi')

//TODO: Variables (Переменные)
// В приложениях нам обычно нужно хранить какие-то значения и работать с ними.
// Например, при создании конвертера валют нам нужно сохранить входное значение, чтобы выполнить соответствующие преобразования. Мы делаем это с помощью переменных, которые являются контейнерами для хранения значений. Переменные создаются с помощью ключевого слова let:
// let name
...
//TODO: Variables (Переменные)
// Мы также можем присвоить нашей переменной значение во время создания:
// let name = 'James'
// let age = 42
...
// Задача: изменить код, чтобы присвоить переменной свое имя и вывести ее.
// let name = 'James'
// console.log(name)

//TODO: Variables (Переменные)
// Переменные могут менять свое значение во время работы программы. Например:
...
// Например, var позволяет повторно объявлять переменные с тем же именем, а let приводит к ошибке.
...
// Константы очень полезны. Если вы используете const, это говорит всем, кто смотрит на ваш код, что это имя никогда не будет присвоено другому значению.
// const color = 'red'
...
// Вот примеры использования математических операторов:
// Задача: изменить значения x и y и посмотреть, как изменятся результаты.
// let x = 8
// let y = 3
...
//TODO: Operator Precedence (Приоритет оператора)
// Приоритет операций в JavaScript такой же, как и в математике: умножение и деление всегда выполняются первыми, затем сложение и вычитание:
// let x = 4 + 5 / 6 - 2
// console.log(x)
...
// Задача: изменить код для расчета и вывода результата прибавления 123 к 321 и умножения результата на 99.
// let x = (4 + 5) / 6 - 2
// console.log(x)

//TODO: Increment (Увеличение)
// Иногда нам нужно многократно добавлять 1 к переменной.
// Например, при подсчете попыток в игре или количестве кликов на веб-странице. Это можно сделать с помощью оператора приращения (++):
// let count = 0
// count++
// console.log(count)
...
// Задача: изменить код для вывода значения переменной после каждого уменьшения.
// let score = 100
// score--
// console.log(score)
// score--
...
// JavaScript также поддерживает сокращения для математических операторов. Например: x = x + 4 эквивалентно x += 4. Вот список всех ярлыков:
// См. Рис: AssignmentOperators.png

//TODO: Practice: Basic Math (Практика: Базовая математика)
// В этом модуле мы сделаем интерактивную форму покупки билетов, чтобы она могла рассчитывать стоимость выбранных билетов и выводить ее.
// В качестве первого шага нам нужно объявить переменные для хранения значений. Задачи:
// 1. Объявите две переменные, одну с именем Adults и другую Children, и инициализируйте их некоторыми значениями, например, 42 и 8.
// 2. Объявите еще одну переменную с именем price, которая будет содержать общую стоимость билетов.
// 3. Рассчитайте стоимость всех билетов, используя следующие цены:
// Взрослые: 12 долларов США,
// Дети: 6 долларов США.
// 4. Вывести значение цены в консоль.
// Подсказка: стоимость рассчитывается по следующей формуле:
...
// Обычно наша программа должна принимать решения на основе некоторых значений. Оператор if указывает блок кода, который мы хотим выполнить, если заданное условие истинно.
// if (condition) {
//   statements
// }
...
// Ознакомьтесь с этой таблицей, чтобы увидеть разбивку операторов сравнения, которые вы можете использовать в условии:
// См. Рис: ComparisonOperators_2.png
...
//TODO: Decision Making (Принятие решения)
// Чтобы проверить дополнительные условия, мы можем использовать оператор if else. Например:
// У вас может быть столько операторов if else, сколько вы хотите.
// Задача: изменить код и добавить в код еще один оператор if else, выводящий «Zero Celsius», если значение temp равно 32.
// let temp = 75
...
//TODO: Combining Conditions (Условия объединения)
// Если мы хотим объединить условия, мы можем использовать логический оператор И (&&):
// Оператор AND вернет true, только если оба условия верны.
// let age = 43
// if (age >= 18 && age < 65) {
//   console.log('Adult')
// }
...
//TODO: Combining Conditions (Условия объединения)
// Точно так же мы можем использовать оператор ИЛИ (||), который возвращает true, если какое-либо из условий истинно. Например:
// Задача: изменить код для вывода «Open», если условие ложно.
// let hour = 19
// let day = 2
...
// В случаях, когда нам нужно присвоить значение на основе условия, мы можем использовать тернарный оператор. Например:
// let hour = 15

// let status = hour > 18 ? 'Closed' : 'Open'

// console.log(status)
// Приведенный выше код присвоит значение «Closed», если условие истинно, и «Open», если нет. Выражение эквивалентно следующему:
// if (hour > 18) {
//   status = 'Closed'
// } else {
//   status = 'Open'
...
//TODO: Practice: Decision Making (Практика: Принятие решения)
// Поскольку мы будем брать количество билетов из полей ввода, нам нужно выполнить некоторую проверку, так как пользователь может ввести неправильные данные, например, отрицательные числа.
// Задача: Для взрослых и детей переменных добавьте оператор if, который проверяет, является ли значение отрицательным. Если это так, присвойте ему значение 0.
// Вот пример оператора if:
// if(adults < 0) {
//   adults = 0;
// }
// Подсказка: проверьте свой код, присвоив переменным отрицательное значение и просмотрев полученную цену.
// let adults = 42
// let children = 8
...
// Это полезно, например, при рисовании анимации в играх или создании обратного отсчета. Цикл for имеет следующий синтаксис:
...
// final-expression (Последнее выражение) - запускается каждый раз после выполнения кода цикла. Обычно он используется для увеличения переменной, используемой в условии.
// Каждый запуск цикла называется iteration (итерацией).
// for (initializer; condition; final - expression) {
//   // code to run
// }

//TODO: Loops (Циклы)
// Вот пример цикла, выводящего числа от 1 до 10 :
// Цикл создает переменную с именем i и инициализирует ее значением 1.
// Затем, после каждой итерации, он увеличивает переменную i на 1.
// Цикл останавливается, когда i достигает 11, нарушая условие.
// Задача: изменить код для вывода чисел от 10 до 50.
// for (let i = 10; i <= 50; i++) {
//   console.log(i)
// }
...
// Мы увеличиваем i на 2 после каждой итерации, в результате чего получаем только четные числа.
// Задача: изменить код, чтобы он выводил только нечетные числа.
...
//TODO: Loops (Циклы)
// Цикл while — это еще один способ создания циклов.
...
//TODO: Loops (Циклы)
// И for, и while могут использоваться для выполнения одной и той же задачи.
// for обычно используется, когда известно, сколько раз нам нужно запустить цикл. Цикл while используется, когда условие цикла более сложное, и количество его запусков зависит от выражения в цикле.
// По возможности используйте циклы for, так как он имеет более чистый синтаксис и его легче читать и понимать.

//TODO: break (сломать)
// Оператор break позволяет преждевременно выйти из цикла.
// Это может быть полезно, например, для выхода из цикла, когда пользователь вводит определенное число или нажимает определенную кнопку. Например:
// Это выйдет из цикла, когда я достигну значения 3.
// Задача: изменить код, чтобы выйти из цикла, когда я достигну числа 8.
// for (let i = 0; i < 10; i++) {
//   if (i == 3) {
//     break
//   }
...
// Оператор continue используется для пропуска одной итерации цикла и продолжения со следующей. Это может быть полезно, например, для пропуска определенных элементов в цикле:
...
//TODO: Practice: Loops (Практика: Циклы)
// Нам нужно создать программу, которая печатает билеты.
// Задача: Создать цикл для взрослых билетов и для каждого билета вывести его номер в следующем формате:
// Ticket #1
// Ticket #2
// ...
// Подсказка: Вот пример цикла :
// for (let i = 1; i <= adults; i++) {
//   //some code
// }
// Для вывода текста и объединения его с номером используйте оператор +, например:
...
//TODO: Arrays (Массивы)
// Массивы позволяют хранить несколько значений в одной переменной. Например, рассмотрим корзину, содержащую несколько товаров. Чтобы хранить все продукты, вам нужно объявить столько переменных, сколько товаров в корзине. С массивами вам нужно только объявить один массив и сохранить в нем все значения.
// Это облегчает поддержку и управление значениями.

//TODO: Arrays (Массивы)
// Для создания массива нам нужно объявить переменную и присвоить ей список значений, заключенных в квадратные скобки:
// Теперь массив корзины включает 3 строковых значения.
// Обратите внимание, что значения разделены запятыми.
// let cart = ['banana', 'apples', 'milk']

//TODO: Arrays (Массивы)
// Чтобы получить доступ к элементу массива, нам нужно использовать его индекс в квадратных скобках.
// Например, чтобы получить доступ к первому элементу:
// Индекс первого элемента равен 0.
// Задача: изменить код для вывода последнего элемента массива.
// let cart = ['banana', 'apples', 'milk']
// console.log(cart[0])
// console.log(cart[2])

// Подобно переменным, мы можем изменить значение элемента:
// Задача: изменить код, чтобы присвоить «bread» 3-му индексу, которого нет в массиве, и посмотреть, что произойдет.
// let cart = ['banana', 'apples', 'milk']
// cart[2] = 'bread'
...
//TODO: Arrays (Массивы)
// Мы можем использовать циклы для перебора массивов.
// Например, выведем все значения массива:
// Во время каждой итерации осуществляется доступ к элементу под индексом x.
// Задача: изменить код для вывода всех элементов с помощью цикла while вместо цикла for.
// let cart = ['banana', 'apples', 'milk']
...
//TODO: Arrays (Массивы)
// В качестве другого примера давайте посчитаем сумму всех значений в массиве:
// Мы объявили переменную res, которая прибавляется к каждому числу в массиве во время итерации цикла.
// Задача: изменить код для вычисления произведения (умножения) всех элементов массива.
...
//TODO: Functions (Функции)
// Давайте создадим функцию под названием «login», которая выводит простое сообщение.
...
// Чтобы вызвать функцию, начните с имени функции, а затем заключите его в круглые скобки. Давайте дважды вызовем нашу функцию login():
// function login() {
//   console.log('Hi!')
// }
...
// Параметры определяются в круглых скобках и могут использоваться как переменные в функции. Например, давайте добавим пользовательский параметр в нашу функцию login() :
// function login(user) {
//   console.log('Hi, ' + user)
// }
...
//TODO: Functions (Функции)
// Во многих случаях нам нужно присвоить результат функции переменной, а не выводить ее.
// Для этого используется оператор return:
// function add(x, y) {
//   return x + y
// }
// Теперь мы можем присвоить переменную результату функции:
// Задача: изменить код для вызова функции add() с параметрами 123 и 321.
// function add(x, y) {
//   return x + y
// }
...
//TODO: Practice: Functions (Практика: Функции)
// Функции полезны, когда нам нужно принимать входные данные и генерировать выходные данные.
// Давайте заменим наш расчет цены вызовом функции!
// Задачи:
// 1. Создать функцию с именем calc (), которая принимает два параметра, взрослые и дети.
// 2. Рассчитать и вернуть из функции цену билета.
// 3. Замените код, присваивающий переменной цены формуле расчета цены, вызов функции.
// Подсказка: Вот как переменной цены должно быть присвоено значение, когда функция calc() определена:
...
//TODO: Events (События)
// Мы также можем создать функцию прямо при определении обработчика события:
// Таким образом, нам не нужно объявлять отдельную функцию и назначать ее событию. Вместо этого мы определяем функцию как обработчик событий, используя этот синтаксис.
// Задача: изменить код для вывода чисел от 1 до 10 в цикле при нажатии кнопки.
{
  /* <button id="myButton">
    Click me
</button> */
...
//TODO: Events (События)
// Теперь, когда мы знаем, как обрабатывать события, давайте создадим конвертер градусов Цельсия в Фаренгейта! Во-первых, нам нужно создать HTML-форму, которая будет содержать два текстовых поля: одно для значения Цельсия, а другое для Фаренгейта:
// Мы также добавили значения по умолчанию в текстовые поля, используя атрибут value.
// Наше приложение автоматически преобразует значение, введенное в любое поле, и покажет результат в другом поле.
{
  /* <form>
...
//TODO: Adding Styles (Добавление стилей)
// Чтобы сделать наше приложение красивым, давайте добавим CSS для стилизации элементов формы:
// Откройте весь код, чтобы увидеть все стили и их результат.
{
  /* <form>
    <label for="celsius">Celsius:</label><br />
    <input type="number" id="celsius" value="0">
...
//TODO: Handing Events (Обработка событий)
// Теперь нам нужно обработать событие ввода обоих текстовых полей и обновить их значения на основе следующей формулы: Fahrenheit = (Celsius * 9/5) + 32
// Вот код:
// window.onload = function () {
...
//   background-color: #303134;
// }
// Мы обрабатываем события oninput для обоих текстовых полей и устанавливаем значение другого поля с помощью формулы.
// Мы создали полностью работающее приложение-конвертер!
// Запустите код, чтобы увидеть его в действии, измените его, чтобы преобразовать другие единицы измерения.

//TODO: Practice: Event Handling (Практика: Обработка событий)
// Пришло время завершить расчет цены и добавить функциональность в нашу форму! Задачи:
// 1. Добавить обработчик события window.onload.
// 2. Внутри нее взять ссылку на кнопку Buy (присвоить ей id, чтобы можно было взять ее ссылку).
// 3. Обработайте событие нажатия кнопки покупки: внутри обработчика возьмите ссылки на текстовые поля для взрослых и детей, используя их идентификаторы.
// 4. Подсчитайте общую стоимость билетов с помощью функции calc ().
// 5. Вывести цену на экран с помощью функции alert().
// Подсказка: возьмите текущее значение, используя .value
// свойство: вот как должен выглядеть ваш обработчик события window.onload :
// window.onload = function () {
...
//   }
// }
// Не забудьте присвоить кнопке «buy» уникальный идентификатор, чтобы вы могли ссылаться на нее из своего кода.
// Поздравляем! Теперь у вас есть работающий сайт продажи билетов — поэкспериментируйте с формой, введите числа и посмотрите итоговую цену.
let adults = 42
let children = 8
