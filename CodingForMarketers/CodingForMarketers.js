...
//TODO: SEO (Search Engine Optimization - Поисковая оптимизация)
// Поисковые системы видят только HTML-код вашей страницы.
// Когда поисковая система сталкивается с вашим контентом, она просматривает HTML-теги сайта, которые определяют, о чем ваш контент и как его классифицировать. В HTML есть теги, которые используются для улучшения SEO ваших веб-страниц, а также для улучшения того, как пользователи видят ваш контент в поисковых системах. Также существуют специальные теги для социальных сетей, определяющие, как ваша страница выглядит в них при публикации. В этом модуле мы изучим основные теги SEO и как их использовать для улучшения SEO наших страниц.
// Теги HTML для SEO влияют на то, как ваш сайт работает в Интернете.
// Без них ваши страницы будут иметь более низкий рейтинг в поисковых системах.

//TODO: HTML (HTML)
// Прежде чем рассматривать теги SEO, давайте сначала разберемся, как устроена веб-страница. Все визуальные элементы веб-страницы помещаются в тег <body> , который действует как основной контейнер веб-страницы. Например:
{
  /* <body>
  <p>this is some content.</p>
</body> */
}
// На страницах также есть раздел <head>, который располагается перед телом и содержит метаинформацию о веб-странице.
// Информация в <head> визуально не представлена ​​на веб-странице, однако она используется поисковыми системами, чтобы понять, о чем ваша страница, и предоставить различную метаинформацию. Об основных метатегах мы узнаем на следующих уроках!
{
  /* <head>
...
// Раздел <head> НЕ совпадает с заголовком страницы. Заголовок является частью тела страницы, а раздел <head> — нет.

//TODO: Meta Title (Мета-заголовок)
// Одним из наиболее важных тегов HTML для SEO является тег <title>. Это метка вашего контента и то, как поиски видят вашу страницу. Заголовок страницы виден на вкладке браузера:
// См. Рис: MetaTitle.png
// <title> Заголовок также используется в поисковых системах для отображения вашей страницы в результатах поиска:
// См. Рис: MetaTitle2.png
// Тег <title> помещается в <head> страницы.
// Например:
{
  /* <head>
...
// Каждая страница сайта может иметь свой уникальный заголовок.
// Важно выбрать описательный заголовок, который правильно описывает веб-страницу и то, о чем она.

//TODO: Meta Description (Мета-описание)
// Следующий метатег — это description (описание).
// Он действует как синопсис на обратной стороне книги. title (Название) — это heading (заголовок), а meta description (метаописание) — это дополнительная пара предложений информации, которые дополнительно описывают содержание.
// Описание создается с помощью тега <meta>:
{
  /* <meta name="description" content="Join Now to learn the basics or advance your existing skills.">  */
}
// Как видите, у meta тега есть атрибут name и атрибут content, который включает в себя текст описания. Описание нигде не видно на странице, однако оно видно поисковым системам: Как видите, это текст, видимый после заголовка.
// См. Рис: MetaDescription.png
// У вас должна быть преемственность между вашим тегом заголовка и вашим мета-описанием, чтобы поисковые системы могли легко распознать, о чем ваша страница.

//TODO: Headings (Заголовки)
// Мы уже видели заголовки в предыдущих модулях.
// Они создаются с помощью тегов от <h1> до <h6> и помогают организовать ваш контент. Помимо организации контента, headings (заголовки) также помогают в SEO. Поисковые системы используют их для определения сегментов контента на вашей странице. Вот иерархия тегов заголовков:
//<h1> - обычно используется для заголовков веб-страниц.
//<h2> - выделяет тему заголовка.
//<h3> - отражает баллы по теме.
//<h4> - поддерживает точки из <h3>.
//<h5> и <h6> - используется не часто, но может использоваться для дальнейшего разбивки точек <h4>.
// Например:
{
  /* <h1>Article Title</h1>
 <h2>A subheading</h2>
...
// На странице может быть несколько тегов от <h1> до <h6>.
// Заголовки позволяют поисковым системам понять иерархию содержания вашей страницы.

//TODO: Images (Картинки)
// Изображения играют важную роль на ваших веб-страницах. По статистике, статьи с изображениями набирают на 94% больше просмотров, чем статьи без них. Изображения создаются с помощью тега <img> и используют атрибут src для ссылки на файл изображения.
{
  /* <img src="https://test.com/image.png" / >  */
}
// Однако поисковые системы не могут определить, что показывают ваши изображения.
// Вот почему атрибут alt важен. Он обозначает альтернативный текст и описывает, о чем изображение:
{
  /* <img src="https://test.com/image.png" alt="A cat playing with fur" /> */
}
// Всегда используйте теги alt на своих изображениях! Помимо поисковых систем, они также используются программами чтения с экрана и когда изображения не могут быть отображены из-за неверных ссылок или плохого соединения.

//TODO: Links (Ссылки)
// Ссылки играют жизненно важную роль в SEO. Они определяют рейтинг вашего сайта в поисковых системах и действуют как «голосование» за вашу страницу.
// Наличие большого количества качественных ссылок на вашу страницу повышает рейтинг страницы. То же самое относится и к исходящим ссылкам. Ссылка на что-то, что не имеет отношения к вашей теме, может привести к штрафам за поиск. Если вам нужна ссылка на страницу, но вы не хотите, чтобы поисковые системы учитывали эту связь, вы можете использовать ссылки «nofollow». Делая это, вы не позволяете внешнему домену, на который вы ссылаетесь, воспользоваться дополнительными полномочиями домена. Вот как создать ссылку «nofollow»:
{
  /* <a href="https://somesite.com/" rel="nofollow">  */
}
// Все, что вам нужно сделать, это добавить атрибут rel со значением «nofollow» к ссылкам, которые вы хотите игнорировать поисковыми системами. Ссылка все равно будет работать, но поисковые системы по ней не перейдут.
// Рассмотрите возможность использования ссылок nofollow на своих страницах, когда вы ссылаетесь на веб-сайт с низким авторитетом домена.
// Это гарантирует, что ссылка не повредит авторитету вашего домена.

//TODO: Open Graph Tags (Теги Open Graph)
// Существуют специальные теги, которые используются социальными сетями.
// Эти теги определяют, как ваша страница должна выглядеть при публикации в социальных сетях, таких как Facebook. Facebook, например, использует теги Open Graph для отображения информации, когда вашим контентом делятся. Теги Open Graph — это метатеги, которые помещаются в <head> вашей страницы. Наиболее популярными являются заголовок, описание и изображение.
// Вот пример тегов открытого графа:
{
  /* <meta property="og:title" content="SoloLearn: Learn to Code">

<meta property="og:description" content="Join Now to learn the basics or advance your existing skills">

<meta property="og:image" content="logo.png">  */
}
// Обратите внимание, что теги используют префикс og: перед их именами. Вот страница, опубликованная на Facebook: у вас может быть одно мета-описание и заголовок для поисковых систем и другое для Facebook. Теги Open Graph дают вам возможность настройки, если ваша веб-страница опубликована в социальных сетях.
// См. Рис: OpenGraphTags.png
// Аналогичные метатеги существуют для Twitter и других социальных сетей.
// Мы не будем охватывать их все, так как важнее знать, как их использовать в случае необходимости, чем запоминать названия. При необходимости вы можете легко найти подходящий метатег в Интернете.

//TODO: Summary (Резюме)
// Поздравляем! Теперь вы знаете самые важные SEO-теги. Эти теги играют важную роль в оптимизации рейтинга вашей страницы в поисковых системах. Без оптимизированных тегов HTML ваше SEO будет страдать. Обязательно всегда проверяйте свои SEO-теги и оптимизируйте их для достижения эффективных результатов SEO. Эти теги включают тег <title>, метаописание, заголовки, теги alt на изображениях, ссылки nofollow и теги открытого графика, используемые для социальных сетей.
// HTML — это язык, который вы можете использовать, чтобы сообщить поисковым системам, о чем ваш веб-сайт и как они должны ранжировать вас.
